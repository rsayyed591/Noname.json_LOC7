    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.data?.error?.message?.includes('model')) {
        return NextResponse.json({ 
          error: 'Invalid model or model not available. Please check the model name and try again.' 
        }, { status: 400 });
      }

      return NextResponse.json(
        { error: error.response?.data?.error?.message || 'An error occurred during analysis' },
